...
import { Search, AlertCircle, ExternalLink, FileText, FileImage } from "lucide-react"

// Extensiones de imagen a probar
const IMAGE_EXTENSIONS = ["jpeg", "jpg", "png", "webp", "jfif", "bmp"]
...
export default function VisualizadorPage() {
  ...
  const handleSearch = () => {
    ...
    if (!normalizedInput.includes(" ")) {
      setError("Formato incorrecto. Por favor ingresa el texto con el formato: SSC GUIA (separados por espacio)")
      return
    }

    const parts = normalizedInput.split(" ")
    if (parts.length !== 2) {
      setError("Formato incorrecto. Debes ingresar exactamente dos valores separados por un espacio")
      return
    }
  ...
  const formatGuiaIfNeeded = (input: string): string => {
    ...
    if (match) {
      ...
      return `${match[1]}-${match[2]}-${match[3]}-${match[4].toUpperCase()}-${match[5]}`
    }
    return input
  }
        ...
        <div className="mx-auto max-w-7xl px-4 py-6 md:px-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-semibold tracking-tight text-foreground">Sistema de Visualización</h1>
                <p className="text-sm text-muted-foreground">SSC y Guías de Despacho</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 md:px-8 md:py-12">
        <div className="mb-10">
          <div className="mb-6 max-w-2xl">
            <h2 className="text-2xl font-semibold text-foreground">Buscar Documento</h2>
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              Ingresa el código SSC y el número de guía separados por un espacio o tabulación. El sistema buscará
              automáticamente los documentos asociados.
            </p>
          </div>
          ...
          <Card className="max-w-3xl border-border bg-card shadow-sm">
            <div className="p-6">
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <Button onClick={handleSearch} size="lg" className="h-11 sm:w-auto sm:px-8">
                  Buscar
                </Button>
              </div>
              ...
              <div className="border-b border-border bg-muted/30 p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="mt-0.5 font-mono text-sm font-semibold text-foreground">{ssc}</p>
                  </div>
                </div>
              </div>
              <div className="p-6">
                <div className="flex flex-col items-center justify-center gap-4 rounded-lg bg-muted/50 py-12">
                  ...
                  <div className="text-center">
                    <h3 className="font-semibold text-foreground">Documento PDF</h3>
                    <p className="mt-1 text-sm text-muted-foreground">Visualizar en navegador</p>
                  </div>
                  <Button
              ...
              <div className="border-b border-border bg-muted/30 p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="mt-0.5 font-mono text-sm font-semibold text-foreground">{guia}</p>
                  </div>
                </div>
              </div>
              <div className="p-6">
                {!imageError ? (
                  ...
                  <div className="flex flex-col items-center justify-center gap-4 rounded-lg bg-muted/50 py-12">
                    ...
                    <div className="text-center">
                      <h3 className="font-semibold text-foreground">Imagen no disponible</h3>
                      <p className="mt-1 text-sm text-muted-foreground">No se encontró la imagen en ningún formato</p>
                    </div>
                  </div>
                )}
              </div>
